...
class JoueursVues:
    def formAddJoueur(self):
        """Formulaire de création de nouveau joeueur
        ...
        while True:
            ...
            if classement == 1:
                break
            jc.add(lastname.lower(), firstname.lower(), birthDate, sexe, classement)
            break

    def addPlayer(self, string, ftTest):
        """Vérifie chaque information entré par l'utilisateur avec la fonction passé en parametre

        Args:
            string ([str]): [string à imprimmer]
            ftTest ([methode]): [vérifie si le format de la string passée en paramètre correspont au format attendu]
    ...
    def listPlayer(self):
        """Affiche le menu et interoge l'utilisateur
        """
        pc = printCustome()
        while True:
            self.backMenu()
    ...
    def backMenu(self):
        """Affiche les options de retour aux menus précédent
        """
        pc = printCustome()
    ...
    def playerUpdate(self):
        """Met à jour le classement du joueur selectionné par l'utilisateur

        Returns:
            [int]: [0 pour retourner au menu principale, 1 pour retourne au menu précédent, 2 tout c'est bien passé]
        """
        jc = JoueursControleurs()
        pc = printCustome()
        while True:
            ...
            if choix.isnumeric():
                if jc.testJoueursInBDD:
                    ...
                    newTable.append(["''" + str(player.doc_id) + "''", player["lastname"], player["firstname"],
                                    player["birthDate"], player["sexe"], str(player["classement"])])
    ...
    def listJoueurDisplay(self):
        """Affiche la liste des joueurs en fortion du choix de l'utilisateur
        ...
        for joueur in result:
            newTable.append(["''" + str(joueur.doc_id) + "''", joueur["lastname"], joueur["firstname"],
                            joueur["birthDate"], joueur["sexe"], str(joueur["classement"])])
        pc.printTable(newTable)

    def listJoueurDisplayFind(self):
        """Recherche les joueurs avec le nom ou prénom demandé par l'utilisateur
